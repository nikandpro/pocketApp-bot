package main

import (
	"log"

	"github.com/boltdb/bolt"
	tgbotapi "github.com/go-telegram-bot-api/telegram-bot-api"
	"github.com/nikandpro/telegram-bot/pkg/repository"
	"github.com/nikandpro/telegram-bot/pkg/repository/boltdb"
	"github.com/nikandpro/telegram-bot/pkg/server"
	"github.com/nikandpro/telegram-bot/pkg/telegram"
	"github.com/zhashkevych/go-pocket-sdk"
)

func main() {
	bot, err := tgbotapi.NewBotAPI("6189858365:AAFEDboNxoi4eEn0hbi6kvllW4tCVbOiz_U")
	if err != nil {
		log.Fatal(err)
	}

	bot.Debug = true

	pocketClient, err := pocket.NewClient("108603-15e9c3ad8ea2a545e85d707")
	if err != nil {
		log.Fatal(err)
	}

	db, err := initDB()
	if err != nil {
		log.Fatal(err)
	}

	tokenRepository := boltdb.NewTokenRepository(db)

	telegramBot := telegram.NewBot(bot, pocketClient, tokenRepository, "http://localhost/")
	
	autorizationServer := server.NewAutorizationServer(pocketClient, tokenRepository, "https://t.me/pocket_bot_golang_telegram_bot")

	go func() {
		if err := telegramBot.Start(); err != nil {
			log.Fatal(err)
		}
	}()

	if err := autorizationServer.Start(); err != nil {
		log.Fatal(err)
	} 

}

func initDB() (*bolt.DB, error) {
	db, err := bolt.Open("pocketBot.db", 0600, nil)
	if err != nil {
		return nil, err
	}

	if err := db.Update(func(tx *bolt.Tx) error {
		_, err := tx.CreateBucketIfNotExists([]byte(repository.AccessTokens))
		if err != nil {
			return err
		}

		_, err = tx.CreateBucketIfNotExists([]byte(repository.RequestTokens))
		if err != nil {
			return err
		}

		return nil
	}); err != nil {
		return nil, err
	}

	return db, nil
}
